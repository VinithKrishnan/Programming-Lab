...
vector<string> getPostFix(vector<string> token_expr)    {
    ...
    for(int i=0;i<token_expr.size();i++)    {
        ...
        else if(temp == "/" or temp == "*" or temp == "-" or temp == "+" or temp == "$" )
            {
...
while(i<input_expr.size())  {
    ...
    if(temp == "+")
    {   if(i==0)
	    	{if(isdigit(input_expr[i+1])||input_expr[i+1]=='(')
	    	   {
	    	   i=i+1;
			   }
			else 
        ...
        else if(i<input_expr.size()-1)
        {
             if((isdigit(input_expr[i+1])||input_expr[i+1]=='(') && (input_expr[i-1]=='/'||input_expr[i-1]=='*') ) {
                i=i+1;
             }
             else   {
                i = i+1;
    ...
    else if(temp == "-")
    {
        ...
        {
             if((isdigit(input_expr[i+1])||input_expr[i+1]=='(') && (input_expr[i-1]=='/'||input_expr[i-1]=='*'||input_expr[i-1]=='('))   {
                postfix_expr.push_back("$");
				i=i+1;
             }
             else   {
